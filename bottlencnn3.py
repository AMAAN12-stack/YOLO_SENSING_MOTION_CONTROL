...
def main():
    ...
    # Debug flags (set True if you want terminal spam)
    ...
    try:
        while True:
            ...
            frame = picam2.capture_array("main")  # RGB888, 640x480
            ...
            if now - last_yolo_time >= YOLO_INTERVAL:
                ...
                if len(results) > 0 and results[0].boxes is not None:
                    ...
                    # Print detections only if debugging (avoid slowing down loop)
            ...
            if bottle_found:
                ...
                # (fix: make ERROR_SMOOTHING behave as comment says)
                filtered_error_x = (
                    (1.0 - ERROR_SMOOTHING) * filtered_error_x
                    + ERROR_SMOOTHING * error_x
                )
                filtered_error_y = (
                    (1.0 - ERROR_SMOOTHING) * filtered_error_y
                    + ERROR_SMOOTHING * error_y
                )
